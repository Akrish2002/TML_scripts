...
def load_npz_TKE_budget(path: str):
...
def plot_TKE_budget(args):
    entries = [load_npz_TKE_budget(f) for f in args.inputs]
    ...
    for idx, (case, time_step, t_normalized, ny, y,
        ...
        if component is not None:
            ...
            if args.zoom:
                ...
                m = (x >= x1) & (x <= x2)
                ax.plot(x[m], y_plot[m], color="r", linestyle=dash_cycle[idx % len(dash_cycle)],
                        linewidth=1.2, label=lab)
            else:
                ax.plot(x, y_plot, color="r", linestyle=dash_cycle[idx % len(dash_cycle)],
                        linewidth=1.2, label=lab)
        else:
            for j, y_plot in enumerate(term_list):
                if len(entries) == 1:
                    term_lab = term_labels[j]
                else:
                    term_lab = f"{lab}, {term_labels[j]}"

                if args.zoom:
                    x1 = args.zoom - args.zoom_window
                    x2 = args.zoom + args.zoom_window
                    m = (x >= x1) & (x <= x2)
                    ax.plot(x[m], y_plot[m], color=colors.get(term_labels[j], "k"),
                            linestyle=dash_cycle[idx % len(dash_cycle)],
                            linewidth=1.2, label=term_lab)
                else:
                    ax.plot(x, y_plot, color=colors.get(term_labels[j], "k"),
                            linestyle=dash_cycle[idx % len(dash_cycle)],
                            linewidth=1.2, label=term_lab)

...
def compute_spectra(args):
    ...
    if T._case.rank == 0:
        ...
        E_uu_kx  = T._E_uu_kx_global
        E_vv_kx  = T._E_vv_kx_global
        E_ww_kx  = T._E_ww_kx_global
        E_TKE_kx = T._E_TKE_kx_global
        ...
        if E_uu_kx.shape[0] != ny:
            raise ValueError(f"E_uu_kx shape mismatch: got {E_uu_kx.shape}, expected ({ny},)")
        if E_vv_kx.shape[0] != ny:
            raise ValueError(f"E_vv_kx shape mismatch: got {E_vv_kx.shape}, expected ({ny},)")
        if E_ww_kx.shape[0] != ny:
            raise ValueError(f"E_ww_kx shape mismatch: got {E_ww_kx.shape}, expected ({ny},)")
        if E_TKE_kx.shape[0] != ny:
            raise ValueError(f"E_TKE_kx shape mismatch: got {E_TKE_kx.shape}, expected ({ny},)")
    
        out_path = Path(args.output_path)
        out_path.mkdir(parents=True, exist_ok=True)
        out_path = out_path / f"kx_spectra_n{ny}_ts{int(args.time_step)}.npz"
        ...
        np.savez(
                    ...
                    E_uu_kx             =   E_uu_kx.astype(np.float64),
                    E_vv_kx             =   E_vv_kx.astype(np.float64),
                    E_ww_kx             =   E_ww_kx.astype(np.float64),
                    E_TKE_kx            =   E_TKE_kx.astype(np.float64)
...
def load_npz_spectra(path: str):
    ...
    print("E_ww_kx shape: ", E_ww_kx.shape)
    print("E_TKE_kx shape: ", E_TKE_kx.shape)
    print("y shape: ", y.shape)
    print("kx shape: ", kx.shape)

    if E_uu_kx.ndim == 1 or E_uu_kx.shape[1] != kx.shape[0]:
        raise ValueError(f"Size error, please check the generated dataset!")
    if E_vv_kx.ndim == 1 or E_vv_kx.shape[1] != kx.shape[0]:
        raise ValueError(f"Size error, please check the generated dataset!")
    if E_ww_kx.ndim == 1 or E_ww_kx.shape[1] != kx.shape[0]:
        raise ValueError(f"Size error, please check the generated dataset!")
...
def plot_spectra(args):
    #Loads only the E_kx along the requested y-locations
    ...
    for idx, (case, t_normalized, ny, y, y_max, kx, E_uu_kx, E_vv_kx, E_ww_kx, E_TKE_kx) in enumerate(entries):
        case_lab = (
                args.labels[idx]
        ...
        for m_idx, m in zip(y_indices, y_delta_multiples):
            for component in components:
                ...
                if k_ref_for_slope is None:
                    k_ref_for_slope = kx[1:]
                    E_ref_for_slope = E_kx[1:]

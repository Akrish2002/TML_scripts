...
def load_npz_TKE_budget(path: str, args, mean_flow_path: str):
    rho_g   = 1.0
    U_l     = 0.0
    U_g     = 3.1830988618379066
    delta_U = U_g - U_l
    delta0 = (2.0 * np.pi) / 100.0
    budget_norm = 1.0 / (rho_g * delta_U**3 / delta0)
    ...
    budget_terms = tuple(term * budget_norm for term in budget_terms)
    ...
    return case, time_step, t_normalized, ny, xi, *budget_terms
...
def plot_TKE_budget(args):
    ...
    entries = [
        load_npz_TKE_budget(TKE_budget_file, args, mf_file)
        for TKE_budget_file, mf_file in zip(args.inputs, args.mean_flow_inputs)
    ]
    ...
    if component is not None:
        ...
        for idx, (case, time_step, t_normalized, ny, y,
            ...
            if args.zoom:
                ...
                mask = (y >= x1) & (y <= x2)
                ax.plot(y[mask], y_plot[mask], color="r", linestyle=dash_cycle[idx % len(dash_cycle)], label=lab)
            else:
                ax.plot(y, y_plot, color="r", linestyle=dash_cycle[idx % len(dash_cycle)], label=lab)

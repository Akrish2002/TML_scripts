...
def first_order_derivative(field, direction, dx, periodic=False):
    ...
    for i in range(length):
        #2.1 Extract directional array
        #2.2 Check if periodic
            #2.2.1 If so perform fully central
            #2.2.2 Else perform combo of central with bounds on edges

        #GPT_Idea
        idx = [slice(None)] * field.ndim
        idx[direction - 1] = i

        temp = field[tuple(idx)]
        temp_derivative = np.empty_like(temp)
    
        if(periodic):
          for j in range(len(temp)):
            if(j == 0):
              temp_derivative[j] = (temp[1] - temp[-1]) / (2*dx)
    
            elif(j == len(temp) - 1):
              temp_derivative[j] = (temp[0] - temp[-2]) / (2*dx)
    
            else:
              temp_derivative[j] = (temp[j + 1] - temp[j - 1]) / (2*dx)
    
        
        else:
          for j in range(len(temp)):
            if(j == 0):
              temp_derivative[j] = (-3*temp[j] + 4*temp[1] - temp[2]) / (2*dx)
    
            elif(j == len(temp) - 1):
              temp_derivative[j] = (3*temp[j] - 4*temp[j - 1] + temp[j -  2]) / (2*dx)
    
            else:
              temp_derivative[j] = (temp[j + 1] - temp[j - 1]) / (2*dx)
            
    
        derivative[tuple(idx)] = temp_derivative
...
def pressure_term(case, dx, dy, dz):
    ...
    #Since this in this particular case I have equal density and incompressible

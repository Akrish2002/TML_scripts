...
class TKE_Budget:
    ...
    def common_terms(self):
        ...
        self._u_org, self._u_avg, self._u_double_prime, self._phi_1, _ = self.read_field_large('u', True)
    ...
    def surface_tension(self):
        ...
        #Using phi = phi_1 to remain consistent with the paper definition
        phi = self._phi_1
        ...
        self._f_sigma_x = f_sigma_x
        self._f_sigma_y = f_sigma_y
        self._f_sigma_z = f_sigma_z

        #Surface tension power in the fluctuation TKE equation
        q_sigma = (
            self._u_double_prime * f_sigma_x +
            self._v_double_prime * f_sigma_y +
            self._w_double_prime * f_sigma_z
        )
    ...
    def compute_spectra_along_kx(self):
       ...
       """
        ...
        E_kx_local = np.zeros((ny_local, nkx_positive), dtype=np.complex128)
        #E_kz_local = np.zeros((ny_local, nkz), dtype=np.float64)

        #This normalization is chosen so that the summed spectral energy matches the
        #plane TKE under numpy's default FFT convention.
        #fft_normalization = float((nx * nz) * (nx * nz))
        ...
        for j in range(ny_local):
            ...
            #uhat_x = np.fft.fft(u_plane, axis = 0)
            #vhat_x = np.fft.fft(v_plane, axis = 0)
            #what_x = np.fft.fft(w_plane, axis = 0)
            ...
            E_kx_local[j, :] = 0.5 * (u_covariance + v_covariance + w_covariance)
            #E_kz_local = 0.5 * (u_covariance + v_covariance + w_covariance)
        ...
        self._E_kx_parts = self._case.comm.gather(E_kx_local, root=0)
        #self._E_kz_parts = self._case.comm.gather(E_xz_kz_local, root=0)

        if(self._case.rank == 0):
            self._E_kx_global = np.concatenate(self._E_kx_parts, axis=0)
            #self._E_kz_global = np.concatenate(self._E_xz_kz_parts, axis=0)

            #Constructing the global y array at cell centers to match the storage
            #format used elsewhere in the script.
            #Dont need this, can add this in the plotting script
            #y_min = self._case.parameters['grid']['y_min']
            #self._spectra_y_global = y_min + (np.arange(self._ny_g) + 0.5) * self._dy

            self._kx_positive_global = kx_positive
            #self._kz_positive_global = kz_positive

        else:
            self._E_kx_global = None
            #self._E_xz_kz_global = None
            #self._spectra_y_global = None

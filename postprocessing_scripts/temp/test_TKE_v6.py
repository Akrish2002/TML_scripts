...
class TKE_Budget:
    ...
    def __init__(self, path):
        ...
        '''
        #1. Setting stack direction as y --> 1
        self._stackdirection = 1

        #2. Timestep to compute the plot for
        self._time_step = 56000

        #3. Setting the path
        self._path = path
        self._ctr_file = str(Path(path) / "incompressible_tml.ctr")

        #4. Have to compute this profile
        #self._mu_1 = 1e-3
        #self._mu_2 = 5e-2
    ...
    def common_terms(self):
        '''
        Pitfalls:
            Getting u', v', w' instead of u'', v'' and w''
                    and Reynolds avg terms instead of Favre avg, 
    ...
    def advection(self):
        ...
        if(self._case.rank == 0):
            ...
            #plot(self._dy, self._advection_global)
    ...
    def pressure_diffusion(self):
        ...
        if(self._case.rank == 0):
            #self._pressure_diffusion_global     = np.concatenate(self._pressure_diffusion_parts, axis=1)
            self._pressure_diffusion_global     = np.concatenate(self._pressure_diffusion_parts)
            '''
            This peforms the overbar from the eqn
            '''
            #self._pressure_diffusion_global = np.mean(self._pressure_diffusion_global, axis=(0, 2))
            #plot(dy, q_global)
    ...
    def turbulent_diffusion(self):
        ...
        if(self._case.rank == 0):
            ...
            #plot(dy, self._turbulent_diffusion_global)
    ...
    def viscous_diffusion(self):
        ...
        if(self._case.rank == 0):
            #print("Viscous diffusion shape: ", viscous_diffusion.shape)
            ...
            #print("Viscous diffusion global shape: ", self._viscous_diffusion_global.shape)
    ...
    def dissipation(self):
        ...
        #mu = self._phi_1 * self._mu_1 + (1 - self._phi_1) * self._mu_2
    ...
    def production(self):
        ...
        if(self._case.rank == 0):
            ...
            #print("Production global shape: ", self._production_global.shape)
    ...
    def plot(self, title="TKE budget terms (plane-averaged)", fname="tke_budget.png", multiple_fields=True):
        if self._case.rank != 0:
            return
        ...
        if not multiple_fields:
            # Keep the previous single-term behavior (advection default)
            if getattr(self, "_advection_global", None) is None:
                raise AttributeError("multiple_fields=False but self._advection_global is None/missing.")
            plt.plot(y, self._advection_global, label="advection")
            plt.title(title)
            plt.xlabel("y")
            plt.grid(True)
            plt.tight_layout()
        ...
        # Explicit ordered list of terms (expects attributes like self._advection_global, etc.)
        terms = [
            ("advection", "advection"),
            ("pressure_diffusion", "pressure diffusion"),
            ("turbulent_diffusion", "turbulent diffusion"),
            ("viscous_diffusion", "viscous diffusion"),
            ("dissipation", "dissipation"),
            ("production", "production"),
            ("surface_tension_induced_diffusion", "surface-tension-induced diffusion"),
            ("sum_all_terms", "Sum of all Terms"),
        ]
    
        # Matplotlib will automatically cycle colors. Use different linestyles/markers for separation.
        linestyles = ["-", "--", "-.", ":", "-", "--", "-."]
        markers = [None, None, None, None, "o", "s", "^"]
    
        plotted_any = False
        for idx, (attr_base, label) in enumerate(terms):
            attr = f"_{attr_base}_global"
            ...
            if arr.ndim != 1 or arr.shape[0] != ny:
                raise ValueError(f"{attr} must be shape (ny,) (or squeezable to it). Got {np.asarray(val).shape}")
            ...
            plt.plot(
                ...
                markevery=max(ny // 32, 1),  # avoid marker clutter
            )
            plotted_any = True
    
        if not plotted_any:
            raise AttributeError("multiple_fields=True but none of the expected '*_global' term arrays were found / non-None.")
...
def main():
    ...
    TKE.plot(fname="tke_budget_n1024.png")
    #TKE.plot()

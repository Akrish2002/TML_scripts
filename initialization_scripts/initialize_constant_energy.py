...
def generate_perturbations(Nz, Ny, Nx, dz, dy, dx, delta, delta_u, seed=0):
    """
    Generate a 3D divergence-free velocity perturbation field (u', v', w')
    with a specified energy spectrum.
    """
    
    #Setting the random seed so it is consistent across runs
    np.random.seed(seed)
    ...
    _, inverse, counts = np.unique(K2, return_inverse=True, return_counts=True)
    Nk = counts[inverse].reshape(K2.shape)

    #This C is the energy in each shell, does not matter since I will normalize it
    #and rescale with amp
    C = 1.0
    ...
    #Debug
    print("amp_u / rms_u: ", amp_u / rms_u)
    print("amp_v / rms_v: ", amp_v / rms_v)
    print("amp_w / rms_w: ", amp_w / rms_w)
